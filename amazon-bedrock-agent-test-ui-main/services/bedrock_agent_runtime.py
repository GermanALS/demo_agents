...
#print(f'{agent_id}, {agent_alias_id}')


def invoke_agent(agent_id, agent_alias_id, session_id, prompt):
    try:
        client = boto3.session.Session().client(service_name="bedrock-agent-runtime", region_name='us-east-2')               #1 Add Region Name
        ...
        response = client.invoke_agent(
            agentId=os.getenv('BEDROCK_AGENT_ID'),                     #2 Add Agent ID
            agentAliasId=os.getenv('BEDROCK_AGENT_ALIAS_ID'),                #3 Add Agent Alias ID
            enableTrace=True,
